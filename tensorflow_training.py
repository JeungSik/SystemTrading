...
class stock_market_dataset:
    ...
    def load_train_data(self, code, type, chart_type, squence_size):
        ...
        if open_index > PREDICT_Y_DATE or last_index < PREDICT_Y_DATE:
            print(
                "주가예측 요청일(" + PREDICT_Y_DATE + ")일이 종목상장일(" + open_index + ") 보다 작거나, 저장된 마지막 주식데이터일(" + last_index + ") 보다 큽니다.")
    ...
    def load_train_group_data(self, codes, type, chart_type, squence_size):
        ...
        for code in codes:
            (train_x, train_y1, train_y2), (test_x, test_y1, test_y2) = self.load_train_data(code, type, chart_type,
                                                                                             squence_size)
...
def create_model():
    model = keras.models.Sequential([
        keras.layers.LSTM(units=64, batch_input_shape=(1, SEQ_LENGTH, DATA_DIM), return_sequences=True,
...
(train_x, train_y1, train_y2), (test_x_t, test_y1_t, test_y2_t) = stock_market.load_train_group_data(LISTS, 'KO', 'DAY',
                                                                                                     SEQ_LENGTH)
(train_x_t, train_y1_t, train_y2_t), (test_x, test_y1, test_y2) = stock_market.load_train_data(CODE, 'KO', 'DAY',
                                                                                               SEQ_LENGTH)
...
while (training_fail):
    ...
    # Training 데이터 학습
    #    history = model.fit(train_x, train_y2, batch_size=BATCH_SIZE, epochs=EPOCH)
    history = model.fit(train_x, train_y2, batch_size=BATCH_SIZE, epochs=EPOCH)

    if history.history['loss'][-1] < 0.01:
        training_fail = False

model.save('./models/rnn_top20_190102.h5')
