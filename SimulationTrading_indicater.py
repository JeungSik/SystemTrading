...
main_form = uic.loadUiType("simulation_trading_indicater.ui")[0]
...
#        sliced_df = self.chart_df.loc[begin_index:self.last_index]
    ...
    def add_sell_sim_df(self, index_date, sell_amount, price, buy_charge, buy_value, buy_price):
        ...
        print("[{}] ({}) 매도 : 수량[{}], 매매가[{}], 수익금[{}], 수익률[{}]".format(name, index_date, int(sell_amount), price, str(format(profit, ',')), str(round(rate * 100, 2)) + '%'))
    ...
    def add_remain_df(self, last_date):
        ...
        print("[{}] ({}) 잔고 : 수량[{}], 현재가[{}], 수익금[{}], 수익률[{}]".format(name, last_date, int(amount), current, str(format(profit, ',')), str(round(rate * 100, 2)) + '%'))
        ...
        print("* 총 {} 종목 잔고 : 손익[ {} ], 수익률[ {} ]".format(str(stock_cnt), str(format(total_profit, ',')), str(round(avg_rate * 100, 2)) + '%'))
...
#            self.clear_simulation_result_table()
    ...
    def search_buy_recommand(self, update_yn):
        ...
        for index in range(start_cnt, total_cnt):
            ...
            # 매수 조건1 실행
            self.sim_buy_condition1(code, type, sim_datas.index[-1], sim_datas.iloc[-1])
