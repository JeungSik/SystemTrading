...
main_form = uic.loadUiType("get_marketinfo_main.ui")[0]
start_dialog = uic.loadUiType("init_marketinfo_dialog.ui")[0]

marketTable_column = {'종목코드': 0, '종목명': 1, '상장일': 2, '감리구분': 3, '종목상태': 4, '구분': 5, '기본': 6,
                      '분봉': 7, '일봉': 8}
upjongTable_column = {'업종코드': 0, '업종명': 1, '구분': 2, '분봉': 3, '일봉': 4}
...
class GetMarketInfo(QMainWindow, main_form):
    def __init__(self):
        ...
        self.save_stocks_min_chartPushButton.clicked.connect(lambda:self.save_stocks_chart_datas('MIN'))

        self.save_upjong_day_chartPushButton.clicked.connect(lambda:self.save_upjong_chart_datas('DAY'))
        self.save_upjong_min_chartPushButton.clicked.connect(lambda:self.save_upjong_chart_datas('MIN'))
        ...
        self.mark_stocks_chart_in_market_table('MIN')

        self.mark_upjong_chart_in_upjong_table('DAY')
        self.mark_upjong_chart_in_upjong_table('MIN')
    ...
    def create_market_table(self):
        ...
        # Progress Bar 생성
        for k in range(marketTable_column['분봉'], marketTable_column['일봉']+1):
            delegate = ProgressDelegate(self.marketTable)
            self.marketTable.setItemDelegateForColumn(k, delegate)
        ...
        for i in range(total_cnt):
            ...
            for k in range(marketTable_column['분봉'], marketTable_column['일봉']+1):
                ...
                self.marketTable.setItem(i, k, progress)
        ...
        self.marketTable.setColumnWidth(marketTable_column['분봉'], 110)
    ...
    def create_upjong_table(self):
        ...
        # Progress Bar 생성
        for k in range(upjongTable_column['분봉'], upjongTable_column['일봉']+1):
            delegate = ProgressDelegate(self.upjongTable)
            self.upjongTable.setItemDelegateForColumn(k, delegate)
        ...
        for i in range(total_cnt):
            ...
            for k in range(upjongTable_column['분봉'], upjongTable_column['일봉']+1):
                progress = QTableWidgetItem()
                progress.setData(QtCore.Qt.UserRole+1000, 0)
                self.upjongTable.setItem(i, k, progress)
        ...
        self.upjongTable.setColumnWidth(upjongTable_column['분봉'], 110)
    ...
    def create_db_chart_note_table(self):
        # 해당 종목의 일봉 테이블 생성
        query = "CREATE TABLE IF NOT EXISTS CHART_NOTE(Code TEXT PRIMARY KEY, DayRecordDate TEXT, MinRecordDate TEXT)"
    ...
    def get_chart_record_date(self, code, type, chart_type):
        ...
        # 해당 종목의 기록일 조회
        if chart_type == 'DAY':
            query2 = "SELECT DayRecordDate FROM CHART_NOTE WHERE Code='"+code+"'"
        elif chart_type == 'MIN':
    ...
    def update_chart_record_date(self, code, type, date, chart_type):
        if chart_type == 'DAY':
            if date < '19850104':
                date = '19850104'

            query1 = "INSERT OR IGNORE INTO CHART_NOTE(Code, DayRecordDate)VALUES('" + code + "', '" + date + "');"
            query2 = "UPDATE CHART_NOTE SET DayRecordDate = '"+ date + "' WHERE Code = '" + code + "';"

        elif chart_type == 'MIN':
            query1 = "INSERT OR IGNORE INTO CHART_NOTE(Code, MinRecordDate)VALUES('" + code + "', '" + date + "');"
            query2 = "UPDATE CHART_NOTE SET MinRecordDate = '" + date + "' WHERE Code = '" + code + "';"
    ...
    def update_progressbar_stocks_chart(self, row, code, type, db_min_date, db_max_date, chart_type):
        ...
        record_date = self.get_chart_record_date(code, type, chart_type)
        ...
        if db_min_date < open_date:
            self.update_chart_record_date(code, type, db_min_date, chart_type)
    ...
    def save_db_stocks_chart_datas(self, row, code, type, chart_datas, chart_type):
        ...
        if self.kiwoom.remained_data == False:
            self.update_chart_record_date(code, type, min_date, chart_type)
            finish = True
...
#            day_record_date = self.kosdaq_cur.fetchone()
    ...
    def mark_progressbar_stocks_chart(self, row, code, type, db_open_date, db_last_date, chart_type):
        now = self.get_now_day()
...
#            self.bug_fix_min_chart_record_date(code, type, datetime.datetime.strftime(db_open_date, '%Y%m%d'))
    ...
    def mark_stocks_chart_in_market_table(self, chart_type):
        if chart_type == 'DAY':
            query = "select A.Code, OpenDate, LastDate " \
                    "from (select TA.Code as Code, case when TA.StockDate < TB.DayRecordDate then TB.DayRecordDate " \
                    "else TA.StockDate end as OpenDate from MARKET_INFO TA " \
                    "left outer join CHART_NOTE TB on TA.CODE = TB.CODE ) A " \
        ...
        kospi_df = DataFrame(kospi_date, columns=['Code', 'OpenDate', 'LastDate'])
        ...
        kosdaq_df = DataFrame(kosdaq_date, columns=['Code', 'OpenDate', 'LastDate'])
        ...
        for i in range(total_cnt):
            ...
            # Progress Bar Update
            self.mark_progressbar_stocks_chart(i, code, type, db_open_date, db_last_date, chart_type)
    ...
    def save_all_datas(self):
        ...
        self.marketTable.selectRow(0)
        self.save_stocks_chart_datas('MIN')
        self.marketTable.selectRow(0)
        self.save_stocks_chart_datas('DAY')
        self.upjongTable.selectRow(0)
        self.save_upjong_chart_datas('MIN')
        self.upjongTable.selectRow(0)
